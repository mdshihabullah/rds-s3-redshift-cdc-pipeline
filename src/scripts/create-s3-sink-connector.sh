...
#########################################################################################################
# Confluent S3 Sink Connector - Creation Script
# Purpose: Create/Recreate the S3 sink connector for MSK Connect to write Parquet files
# Usage: ./create-s3-sink-connector.sh [--delete-existing]
#########################################################################################################
...
CONNECTOR_NAME="S3SinkConnectorParquet"
PROPERTIES_FILE="s3-sink-connector-config.properties"
...
discover_aws_config() {
    ...
    # 7. Find Confluent S3 Sink Plugin
    print_info "  [7/7] Finding Confluent S3 Sink Plugin..."
    
    local plugins=$(aws kafkaconnect list-custom-plugins --region ${AWS_REGION} --query 'customPlugins[?contains(name, `S3`) || contains(name, `s3`) || contains(name, `Sink`) || contains(name, `Confluent`)].{Name:name,Arn:customPluginArn,Revision:latestRevision.revision}' --output json 2>/dev/null)
    
    if [[ -z "${plugins}" ]] || [[ "${plugins}" == "[]" ]]; then
        print_error "    No Confluent S3 Sink plugin found"
    ...
    if [[ $plugin_count -eq 1 ]]; then
        ...
        print_warning "    Found ${plugin_count} S3 plugins. Please select one:"
        echo ""
...
monitor_connector_status() {
    ...
    while [[ $attempt -lt $max_attempts ]]; do
        ...
        if [[ "${state}" == "RUNNING" ]]; then
            ...
            echo ""
            print_info "The connector will start consuming from Kafka topics and writing Parquet files to S3"
            print_info "S3 Bucket: $(grep '^s3.bucket.name=' ${PROPERTIES_FILE} | cut -d'=' -f2)"
            print_info "Base Path: $(grep '^topics.dir=' ${PROPERTIES_FILE} | cut -d'=' -f2)"
            echo ""
...
main() {
    print_header "Confluent S3 Sink Connector Setup (Parquet Format)"
    echo ""
    
    print_info "Note: This connector will write CDC events as Parquet files to S3"
    echo ""
    ...
    if create_connector "${connector_config}"; then
        ...
        if monitor_connector_status "${CONNECTOR_NAME}"; then
            ...
            echo ""
            print_success "S3 Sink Connector is now running and writing Parquet files"
            echo ""
            print_info "Next steps:"
            echo "  1. Verify Parquet files are being written to S3:"
            echo "     aws s3 ls s3://$(grep '^s3.bucket.name=' ${PROPERTIES_FILE} | cut -d'=' -f2)/$(grep '^topics.dir=' ${PROPERTIES_FILE} | cut -d'=' -f2)/ --recursive"
            echo ""
